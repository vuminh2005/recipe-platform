...
class RecipeCatalogApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_public_list_contains_both_ml_recipes_but_not_hello(
        self,
    ) -> None:
    ...
    async def test_tabular_detail_contains_complete_supported_contract(
        self,
    ) -> None:
        response = await self.get("/api/recipes/tabular-random-forest")
        ...
        self.assertEqual(
            ...
            "binary_tabular_classification",
        )
        self.assertEqual(definition["framework"], "scikit_learn")
        self.assertEqual(definition["model"], "RandomForestClassifier")
        self.assertEqual(
        ...
        self.assertEqual(
            {
                field["name"]
                for field in definition["configurable_search_space"]
            },
    ...
    async def test_public_metadata_contains_no_private_execution_fields(
        ...
        for recipe_id in ("cats-dogs", "tabular-random-forest"):
            definition = (await self.get(f"/api/recipes/{recipe_id}")).json()
